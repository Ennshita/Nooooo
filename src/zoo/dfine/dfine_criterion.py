...
class DFINECriterion(nn.Module):
    ...
    def loss_local(self, outputs, targets, indices, num_boxes, T=5):
        ...
        losses = {
            "loss_fgl": torch.tensor(0.0, device=device, requires_grad=True),
            "loss_ddf": torch.tensor(0.0, device=device, requires_grad=True),
        }
        ...
        if "pred_corners" in outputs:
            ...
            if len(idx[0]) > 0:
                ...
                if "teacher_corners" in outputs:
                    ...
                    if torch.equal(pred_corners_all, target_corners_all):
                        ...
                        num_pos = self.num_pos if (self.num_pos is not None and self.num_pos > 0) else 1.0
                        num_neg = self.num_neg if (self.num_neg is not None and self.num_neg > 0) else 1.0
                        loss_match_local1 = loss_match_local[mask].mean() if mask.any() else torch.tensor(0.0, device=device, requires_grad=True)
                        loss_match_local2 = loss_match_local[~mask].mean() if (~mask).any() else torch.tensor(0.0, device=device, requires_grad=True)
                        losses["loss_ddf"] = (
    ...
    def _get_src_permutation_idx(self, indices):
        ...
        if len(indices) == 0 or all(len(src) == 0 for (src, _) in indices):
    ...
    def _get_tgt_permutation_idx(self, indices):
        ...
        if len(indices) == 0 or all(len(tgt) == 0 for (_, tgt) in indices):
            return torch.zeros((0,), dtype=torch.long), torch.zeros((0,), dtype=torch.long)
    ...
    def get_loss_meta_info(self, loss, outputs, targets, indices):
        ...
        idx = self._get_src_permutation_idx(indices)
        if len(idx[0]) == 0:
            src_boxes = outputs["pred_boxes"].new_zeros((0, outputs["pred_boxes"].shape[-1]))
            target_boxes = outputs["pred_boxes"].new_zeros((0, 4))
        else:
            src_boxes = outputs["pred_boxes"][idx]
